...
The ZeroGPU MCP server is hosted at `https://mcp.zerogpu.ai/mcp` and authenticates with two headers on every request: `x-api-key` (your API key) and `x-project-id` (your Project ID). To make its tools available to Claude, add the server to Claude Desktop's MCP configuration.

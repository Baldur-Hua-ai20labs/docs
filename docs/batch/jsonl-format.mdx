...
upload it via [`POST /v1/files`](/api-reference/upload-file)
with `purpose=batch`, then reference its ID when you call
[`POST /v1/batches`](/api-reference/create-batch).
...
download it via
[`GET /v1/files/{file_id}/content`](/api-reference/download-file).

...
<CardGroup cols={2}>
  ...
  <Card title="Supported endpoint" href="/api-reference/batch/supported-endpoints">
    Only `/v1/chat/completions` is batchable. Body and response shape.
  </Card>
  ...
  <Card title="Create batch (playground)" href="/api-reference/batch/create-batch">
    Try `POST /v1/batches` after you have an input file id.
  </Card>
  <Card title="Retrieve batch (playground)" href="/api-reference/batch/retrieve-batch">
    Poll `GET /v1/batches/{batch_id}` for status and file ids.
  </Card>
</CardGroup>
